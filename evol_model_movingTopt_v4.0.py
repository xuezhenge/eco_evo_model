...
parser.add_argument('--vG_A0', type=float,
    default=0.7, help='0,0.075,0.15,0.3,0.6,1.2')
...
#vG_A0 = args.vG_A0; vG_L = args.vG_L; vE_A = args.vE_A; vE_L = args.vE_L
# parameters
q1A = 1.5
K_A = 50000000
    ...
    for i in np.arange(1,n_t):
        ...
        if Temp_t_cur < ToptA_t - 15 and num_change_A == 0:
            ...
            def get_AL(t,A_t,L_t):
                ...
                A[i] = A_t*denomA
                L[i] = L_t*denomL
                # print(i,Temp_t_cur,new_ToptA,new_vG_A,new_ToptL,w_A[i],w_L[i],A[i],L[i])
    ...
    while year <= num_years:
        if task == 'SA':
            ...
            if model == 'C+P+G':
                ...
                if year <= 20:
                    vG_A0 = 0.3; vG_L = 0.3; vE_A = 0.7; vE_L = 0.7
                else:
                    vG_A0 = args.vG_A0; vG_L = args.vG_L; vE_A = args.vE_A; vE_L = args.vE_L
        # genetic parametes
        ...
        print(model)
        print(f'year{year}: vG_A{vG_A0}_vG_L{vG_L}_vE_A{vE_A}_vE_L{vE_L}')
        ...
        if year >= num_years:
            AL = np.vstack([Adens_p,Ldens_p,ToptAss,ToptLss,vG_As])
            AL = pd.DataFrame(np.transpose(AL))
            AL.columns = ['Aden', 'Lden', 'ToptA', 'ToptL','vG_A']
...
folder_list = ['data','A','L','ToptAL','vGA']
loc_dir_list = []
for folder in folder_list:
    if task == '9points' or task == 's0t1' or task == 's1t0':   
        ...
        vG_A0 = args.vG_A0; vG_L = args.vG_L; vE_A = args.vE_A; vE_L = args.vE_L
        loc_dir = os.path.join('../../outputs', f'AdaptedTopt_{num_years}years_{climate}_{task}_{code_version}/{model}/ov{ov_mor}_vGA{vG_A0}_vGL{vG_L}_vEA{vE_A}_vEL{vE_L}_steps{num_steps}',folder)
    elif task == 'SK_SA':
        vG_A0 = args.vG_A0; vG_L = args.vG_L; vE_A = args.vE_A; vE_L = args.vE_L
        loc_dir = os.path.join('../../outputs', f'AdaptedTopt_{num_years}years_{climate}_{task}_{code_version}/{model}/ov{ov_mor}_vGA{vG_A0}_vGL{vG_L}_vEA{vE_A}_vEL{vE_L}_steps{num_steps}/s{seas}_k{trend}',folder)
