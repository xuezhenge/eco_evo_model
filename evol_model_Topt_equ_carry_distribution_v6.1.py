...
def find_equ(fn):
    ...
    # because aphid reaches the equilibrim eariler then ladybird, the estimated ToptL is more accurrate than aphid, so we will do several extral test to check the estimated ToptA is accurate or not
    if equilibriumA == 1 and equilibriumL==1:
        if kA < 0:
            ...
            print(kA,kA_down1,kA_down2)
            print(ToptA0,ToptA0_down1,ToptA0_down2)
            if abs(kA) < abs(kA_down1) and abs(kA) < abs(kA_down2):
                ToptA0 = ToptA0
            elif abs(kA_down1) < abs(kA) and abs(kA_down1) < abs(kA_down2):
                ToptA0 = ToptA0_down1
            elif abs(kA_down2) < abs(kA) and abs(kA_down2) < abs(kA_down1):
                ToptA0 = ToptA0_down2
            print(f"ToptA_eqn = {ToptA0}")
            ...
            print(kA,kA_up1,kA_up2)
            print(ToptA0,ToptA0_up1,ToptA0_up2)
            if abs(kA) < abs(kA_up1) and abs(kA) < abs(kA_up2):
                ToptA0 = ToptA0
            elif abs(kA_up1) < abs(kA) and abs(kA_up1) < abs(kA_up2):
                ToptA0 = ToptA0_up1
            elif abs(kA_up2) < abs(kA) and abs(kA_up2) < abs(kA_up1):
                ToptA0 = ToptA0_up2
            print(f"ToptA_eqn = {ToptA0}")
    ...
    if task == 's1t0':
        Topts.to_csv(f'../../inputs/{climate}{equ_year}_{task}/Topt_equ_ov{ov_mor}_vGA{vG_A0}_vGL{vG_L0}_vEA{vE_A}_vEL{vE_L}_{code_version}/{fn}',index=False)
    elif task == 'SK_SA':
        Topts.to_csv(f'../../inputs/{climate}{equ_year}_{task}/Topt_equ_ov{ov_mor}_vGA{vG_A0}_vGL{vG_L0}_vEA{vE_A}_vEL{vE_L}_{code_version}/{lon}_{lat}_{seas}.csv',index=False)
    else:
        Topts.to_csv(f'../../inputs/{climate}{equ_year}/Topt_equ_ov{ov_mor}_vGA{vG_A0}_vGL{vG_L0}_vEA{vE_A}_vEL{vE_L}_{code_version}/{fn}',index=False)
    print('file saved!!!')
    toc = time.perf_counter()
    print(f'total time for finding equilibrium: {(toc-tic)/3600} hours')

if task == 's1t0' or task == 'SK_SA':
    loc_dir = f'../../inputs/{climate}{equ_year}_{task}/Topt_equ_ov{ov_mor}_vGA{vG_A0}_vGL{vG_L0}_vEA{vE_A}_vEL{vE_L}_{code_version}'
else:
    loc_dir = f'../../inputs/{climate}{equ_year}/Topt_equ_ov{ov_mor}_vGA{vG_A0}_vGL{vG_L0}_vEA{vE_A}_vEL{vE_L}_{code_version}'
